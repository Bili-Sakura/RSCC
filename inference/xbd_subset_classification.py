...
# nohup python inference/xbd_subset_classification.py > ./logs/subset_classification.log &

# Set up logging
log_file = "./logs/subset_classification.log"
...
image_dir = "./data/xbd/xbd_subset/{event_name}/images/"
# Check if the output file already exists and read processed items
output_file_path = "./output/xbd_subset_classification.jsonl"

# Prompt Template
prompt_template = "Which natural distaster does these 2 satellite images show? Choose one from 'earthquake', 'wildfire', 'volcano eruption', 'storm', 'flooding', and 'tsunami'. Output your answer in json with key as 'event'. "
...
for idx, image_pair in enumerate(tqdm(image_pairs, desc="Processing")):
    ...
    for model_id, device in zip(MODEL_LIST, device_mapping):
        ...
        classification_result = (
            re.search(r'"event":\s*"([^"]+)"', result).group(1)
            if re.search(r'"event":\s*"([^"]+)"', result)
            else "unknown"
        )
        item = {
            "model_id": model_id,
