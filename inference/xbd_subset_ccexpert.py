...
# nohup python inference/xbd_subset_ccexpert.py > ./logs/ccexpert.log &

# Set up logging
log_file = "./logs/ccexpert.log"
...
output_file_path = "./output/xbd_subset_ccexpert.jsonl"
...
MODEL_ID = "Meize0729/CCExpert_7b"
DEVICE = "cuda:0"
CCEXPERT_MODEL = ccexpert_model_loader(MODEL_ID, DEVICE)
...
for idx, image_pair in enumerate(tqdm(image_pairs, desc="Processing")):
    ...
    items = []
    result = inference_ccexpert(
        model_id=MODEL_ID,
        text_prompt=prompt_template,
        pre_image=pre_image,
        post_image=post_image,
        model_hub=CCEXPERT_MODEL,
    )
    item = {
        "model_id": MODEL_ID,
